...
public class ChunkManager : Singleton<ChunkManager>
{
    ...
    private Dictionary<Vector2Int, VoxelChunk> chunkDict = new Dictionary<Vector2Int, VoxelChunk>();
    ...
    /// </summary>
    private void initRegion(int initX, int initZ)
    {
    ...
    /// </summary>
    private void LoadRegion(int initX, int initZ)
    {
    ...
    /// </summary>
    private void HideRemoveChunk()
    {
        List<Vector2Int> removeList = new List<Vector2Int>(); ;
        foreach (KeyValuePair<Vector2Int, VoxelChunk> chunk in chunkDict)
        {
    ...
    /// </summary>
    private void LoadChunkFromList()
    {
        ...
        GameObject chunkObj = new GameObject($"Chunk[{key.x}, {key.y}]", typeof(MeshFilter), typeof(MeshRenderer))
        {
            transform =
                {
                    parent = transform,
                    position = new Vector3(key.x * Constants.CHUNK_SIDE, 0, key.y * Constants.CHUNK_SIDE)
                }
        };
        ...
        if (chunkIndexInRegion != 0)//Load chunk from a region data
            chunkDict.Add(key, chunkObj.AddComponent<VoxelChunk>().ChunkInit(regionDict[regionPos].GetChunkData(chunkIndexInRegion), keyInsideChunk.x, keyInsideChunk.y, regionDict[regionPos], false));
        else //Generate chunk with the noise generator
            chunkDict.Add(key, chunkObj.AddComponent<VoxelChunk>().ChunkInit(noiseManager.GenerateChunkData(key), keyInsideChunk.x, keyInsideChunk.y, regionDict[regionPos], Constants.SAVE_GENERATED_CHUNKS));
    ...
    {
        //save chunks
        foreach (VoxelChunk chunk in chunkDict.Values)
